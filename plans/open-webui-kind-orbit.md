...
2. **Объединение БД задач и AI-memory** (упомянуто пользователем). Сейчас задачи хранятся в `pm-mcp-server/.../db.sqlite3`, memory в `ai-memory/data/memory.db`. Перенос — отдельный план в духе [DONE-global-numbering-migration.md](D:\GitHub\_engineering_rules\Plans\DONE-global-numbering-migration.md).
