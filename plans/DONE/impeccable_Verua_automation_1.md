Контекст загружен — PRODUCT.md/DESIGN.md отсутствуют. По правилам сначала читаю AGENTS.md (память требует это до любой работы) и проверяю наличие shared Design-system.

Прочитал AGENTS.md. Shared Design-system есть и авторитетна — читаю её. Также проверяю recent memory как требует AGENTS.md.

Контекст полный: Verua_automation — FastAPI/Jinja2 медицинская автоматизация на shared MD3 Design-system. PRODUCT.md/DESIGN.md не нужен — AGENTS.md+DESIGN_SYSTEM.md дают эквивалент. Аудит — анализ, задачу в Assistant-UI не создаю (K.2).

Начинаю параллельную разведку: основные шаблоны и static CSS.

Уже вижу серьёзные несоответствия с MUST NOT правилами Design-system. Продолжаю аудит.

Достаточно картины из CSS. Проверяю шаблоны на использование MD3 web-компонентов и делаю grep по нарушениям.

Собрал все нужные данные. Формирую отчёт.
